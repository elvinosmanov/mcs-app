...
export default function Events() {
  ...
  const projects = ['Production Line A', 'Production Line B', 'Assembly Line C'];
  const devices = ['Temperature Sensor 1', 'Pressure Sensor 2', 'Flow Meter 1'];
  ...
  const filteredEvents = events.filter(event => {
    const matchesProject = filters.project ? event.project === filters.project : true;
    const matchesDevice = filters.device ? event.device === filters.device : true;
    const matchesEventType = filters.eventType ? event.eventType === filters.eventType : true;
    const matchesSeverity = filters.severity ? event.severity === filters.severity : true;
    const matchesSearch = filters.search ? 
      event.description.toLowerCase().includes(filters.search.toLowerCase()) ||
      event.device.toLowerCase().includes(filters.search.toLowerCase()) : true;
    
    const inDateRange = filters.dateRange.start && filters.dateRange.end ?
      new Date(event.timestamp) >= new Date(filters.dateRange.start) &&
      new Date(event.timestamp) <= new Date(filters.dateRange.end) : true;

    return matchesProject && matchesDevice && matchesEventType && 
           matchesSeverity && matchesSearch && inDateRange;
  });
  ...
  const sortedEvents = [...filteredEvents].sort((a, b) => {
    ...
    const aValue = a[sortConfig.key] instanceof Date ? 
      a[sortConfig.key].getTime() : 
      a[sortConfig.key];
    const bValue = b[sortConfig.key] instanceof Date ? 
              ...
              key !== 'id' && (
                <div key={key}>
                  ...
                  <p className="mt-1 text-sm text-gray-900">
                    {value instanceof Date ? format(value, 'MMM d, yyyy HH:mm:ss') : value}
                  </p>
                </div>
              )
            ))}
